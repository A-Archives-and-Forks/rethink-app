...
class RethinkPlusViewModel(application: Application) : AndroidViewModel(application), KoinComponent {
    ...
    private fun observeSubscriptionState() {
        viewModelScope.launch {
            InAppBillingHandler.getSubscriptionStateFlow().collect { state ->
                if (persistentState.rpnTestWithDefPurchase) {
                    handleSubscriptionStateChange(SubscriptionStateMachineV2.SubscriptionState.Active)
                    Logger.d(LOG_IAB, "$TAG: Subscription state changed to ${SubscriptionStateMachineV2.SubscriptionState.Active.name}")
                } else {
    ...
    fun setProducts(productList: List<ProductDetail>) {
        ...
        // of showing the purchase UI.  Never silently return — that leaves the UI
        // stuck in Loading shimmer.
        ...
        when (state) {
            ...
            is SubscriptionStateMachineV2.SubscriptionState.Active -> {
                ...
                if (purchaseFlowActive) {
                    ...
                    if (current is SubscriptionUiState.Loading) {
                        _uiState.value = SubscriptionUiState.AlreadySubscribed(
                            RpnProxyManager.getRpnProductId() ?: ""
                        )
                    }
    ...
    private fun startPendingPurchasePolling() {
        ...
        pollingJob = viewModelScope.launch(Dispatchers.IO) {
            while (isActive) {
                ...
                // Query entitlement from server once after three attempts of fetchPurchases.
                if (counter == 3) {
                    ...
                    if (purchaseToken == null) {
                        ...
                        InAppBillingHandler.acknowledgePurchaseFromServer(accountId, deviceId, purchaseToken, productType)
                    }
                }
                counter++
